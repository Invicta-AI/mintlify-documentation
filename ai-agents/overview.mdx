...
<Note>
  **AI Agents** are autonomous units designed to perform tasks, make decisions,
  and communicate with other agents. Think of an AI Agent as a team member with
...
- **Files**: Store various file types such as PDFs, TXT, CSV, etc.
- **Links**: Save and manage URLs for easy access.
- **Memories**: Retain interactions directly from chat for future reference.
- **Integrations**: Connect with external services to enrich the knowledge base.
...
Upload a CSV file and run the AI agent on each row of the spreadsheet. This mode is ideal for performing batch operations easily.

<Card title="Batch Mode" icon="table" href="/ai-agents/batch-mode">
  Explore how to use Batch Mode for efficient task automation.
</Card>
...
<Steps>
  <Step title="Access the Agent Builder">
    Click on 'Explore AI Agents' or the profile menu button, then click on 'Create
    new AI Agent'.
  </Step>
